...
class ControllerNode(Node):
    def __init__(self):
        ...
        #Set param
        self.declare_parameter('display', 'monitor1')
        ...
        if '/monitor1/' in namespace:
            self.create_subscription(Twist, "/monitor1/cmd_vel", self.get_monitor_cmd_vel_callback, 10)
            self.complete_path_client = self.create_client(SetBool, "/num_complete_path")
            self.num_complete_path = 0;
            # Inside your __init__ method
            self.switch_state_monitor2_client = self.create_client(
                ConfigureController, '/monitor2/switch_state'
            )
    ...
    def keyboard_server_callback(self, request:ConfigureController, response:ConfigureController):
        ...
        if '/monitor1/' in self.namespace :
            ...
            if key=='m':
                ...
                #self.queue = np.array(self.position_data[::-1])
                print(f"Set {self.queue}")
                # print(self.get_path1)
                # print("-------------------")
                # print(self.get_path2)
                # self.current_state == "IDLE"
                # self.switch_state("IDLE")
        if key == 'r':
            self.num_usable_pizza = 150
            self.path_index = 0
            
        if key == 'o':
            self.current_state = "SAVE"
            self.switch_state("SAVE")
            self.get_logger().info(f"Stack {self.path_index+1}")
            if self.path_index < 4:
                path_key = f'path{self.path_index + 1}'
                self.yaml_paths[path_key].extend(self.position_data)
                self.save_yaml(self.yaml_paths)
                self.position_data = []
                self.path_index += 1
                self.get_logger().info(f"Saved positions to {path_key}")

                # After saving 4 paths, switch to PLAY4
            elif self.path_index == 4:
                self.get_logger().info(f"Set {self.queue} for PLAY4")
                self.switch_state_monitor2('PLAY4')
                self.current_state = "PLAY4"
                self.switch_state("PLAY4")

            else:
                self.get_logger().info("Can't save, full paths storage")
            self.current_state = "IDLE"
            self.switch_state("IDLE")
    ...
    def switch_state_monitor2(self, state):
        state_request = ConfigureController.Request()
        state_request.name = str(state)
        self.switch_state_monitor2_client.call_async(state_request)
    ...
    def timer_callback(self):
        ...
        #     self.current_state == "IDLE"
        #     self.switch_state("IDLE")
        if self.current_state == "IDLE" or self.current_state == "DRAW" :
            self.cmdvel(float(self.m1_cmd_vel_linear),float(self.m1_cmd_vel_angular))
            
        elif self.current_state == "CLEAR":
            self.walk_eat_until_end()
            
        elif self.current_state == "PLAY" and '/monitor1/' in self.namespace:
            self.walk_spawn_until_end()
            
        elif self.current_state == "PLAY4" and '/monitor2/' in self.namespace:
